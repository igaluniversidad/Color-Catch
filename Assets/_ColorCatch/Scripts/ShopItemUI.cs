...
public class ShopItemUI : MonoBehaviour
{
    ...
    {
        if (PaletteManager.Instance == null || currentPalette == null) return;

        // 1. SI YA ESTÁ DESBLOQUEADA -> Solo equipar (Sonido y vibración de clic normal)
        ...
        // 2. SI ESTÁ BLOQUEADA -> Intentar comprar
        else
        {
            bool purchased = PaletteManager.Instance.TryBuyPalette(currentPalette);

            if (purchased)
            {
                // Sonido y vibración de compra exitosa
                AudioManager.Instance?.PlayPurchaseSuccess();
                HapticManager.Instance?.TriggerMilestoneFeedback();
            }
            else
            {
                // Sonido/vibración de fallo si no alcanzan las estrellas
                AudioManager.Instance?.PlayButtonClick();
                HapticManager.Instance?.TriggerFailureFeedback();
            }
        }
    }
}